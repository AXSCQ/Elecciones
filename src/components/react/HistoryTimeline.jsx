...
export default function HistoryTimeline() {
  ...
  // IMPORTANTE: Necesitas agregar este useState en tu componente padre:
    ...
    ${isExpanded ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
      ...
      transform transition-transform duration-300 ease-in-out"
                    ...
                    <div className="hidden md:flex md:flex-col md:items-center relative group w-full md:w-auto">
                      ...
                      <div className={`absolute ${isEven ? 'top-full mt-8' : 'bottom-full mb-8'}
                      ${isFirst ? 'left-0' : isLast ? 'right-0' : 'left-1/2 transform -translate-x-1/2'}
                      w-80 xl:w-96 overflow-hidden transition-all duration-500 ease-in-out z-40
                      ${isExpanded ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
